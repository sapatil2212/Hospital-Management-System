...
import {
  ...
  Shield, Key, Camera, Phone,
} from "lucide-react";
import { useDoctorDashboard } from "../DoctorDashboardContext";

export default function DoctorProfilePage() {
  const router = useRouter();
  const { doctor, setDoctor, accent, doctorName, deptName, initials } = useDoctorDashboard();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.data) {
          setDoctor({ ...doctor, ...data.data, profileImage: profilePhoto });
        }
      } else {
    ...
    <div style={{ fontFamily: "'Inter', sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 20, alignItems: "start" }}>
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #d1fae5", overflow: "hidden", boxShadow: "0 1px 4px rgba(16,185,129,0.04)" }}>
          <div style={{ background: `linear-gradient(135deg,${accent},#0ea5e9)`, padding: "28px 24px", textAlign: "center" }}>
            <div
