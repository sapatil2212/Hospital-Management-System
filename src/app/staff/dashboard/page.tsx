...
import {
  ...
  ChevronRight, CalendarDays, Users
} from "lucide-react";
      ...
      <header style={{ height: 64, background: "#fff", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 24px", position: "sticky", top: 0, zIndex: 40, boxShadow: "0 1px 4px rgba(0,0,0,.04)", flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ textAlign: "right", marginRight: 4 }}>
            ...
            <div style={{ fontSize: 11, color: "#94a3b8" }}>{ROLE_LABELS[profile.role] || profile.role} · {profile.hospital?.name}</div>
          </div>
          ...
          <button onClick={() => router.push("/staff/change-password")} style={{ width: 36, height: 36, borderRadius: 10, background: "#f8fafc", border: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }} title="Change Password">
            <Key size={15} color="#64748b" />
          </button>
          <button onClick={handleLogout} style={{ width: 36, height: 36, borderRadius: 10, background: "#fff5f5", border: "1px solid #fee2e2", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer" }} title="Logout">
                  ...
                  <>
                    ...
                    {patients.map(p => (
                      <div key={p.id} className="sd-pt-row">
                        ...
                        <div>
                          <a href={`/hospitaladmin/patients/${p.id}`} target="_blank" rel="noreferrer" style={{ fontSize: 12, color: "#3b82f6", fontWeight: 600, textDecoration: "none", display: "inline-flex", alignItems: "center", gap: 4, padding: "5px 10px", borderRadius: 7, background: "#eff6ff", border: "1px solid #bfdbfe" }}>
                            View Profile
                          </a>
                        </div>
                      </div>
